...
process and one port. One static Go binary.
...
- **No live reload.** Project and site are read once at startup; restart after
  a rebuild.
- **Zones are access, not visibility.** Every zone serves the same pages; a
  restricted one only asks who you are. Documentation that must differ is a
  separate build. See [AUTH.md](./AUTH.md).
